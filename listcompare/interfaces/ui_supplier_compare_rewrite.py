...
def _conflict_candidate_rows_text(candidate: SupplierConflictCandidate) -> str:
    if len(candidate.source_row_numbers) == 1:
        return f"Rad {candidate.source_row_numbers[0]}"
    row_numbers = ", ".join(str(row_number) for row_number in candidate.source_row_numbers)
    return f"Rader {row_numbers}"


def _conflict_candidate_option_label(candidate: SupplierConflictCandidate) -> str:
    rows_text = _conflict_candidate_rows_text(candidate)
    if len(candidate.source_row_numbers) == 1:
        return rows_text
    return f"{rows_text} ({len(candidate.source_row_numbers)} identiska rader)"
...
def _render_prepare_conflict_group(
    *,
    ...
    for candidate_index, candidate in enumerate(conflict.candidates, start=1):
        option_label = f"Alternativ {candidate_index}: {_conflict_candidate_option_label(candidate)}"
        option_labels[candidate.candidate_id] = option_label
        candidate_rows.append(
            {
                "Alternativ": f"Alternativ {candidate_index}",
                "Källrad(er)": _conflict_candidate_rows_text(candidate),
    ...
    if selection_key not in st.session_state and conflict.group_key in stored_choices:
        st.session_state[selection_key] = stored_choices[conflict.group_key]
