...
from .session.shared_sync_status import (
    maybe_run_auto_shared_sync as _maybe_run_auto_shared_sync,
    store_shared_sync_status as _store_shared_sync_status,
)
...
def main() -> None:
    ...
    if "_auto_shared_sync_cache" not in st.session_state:
        st.session_state["_auto_shared_sync_cache"] = {}
    shared_sync_status = _maybe_run_auto_shared_sync(
        st.session_state,
        sync_runner=_sync_shared_files,
    )
    _init_session_state(st.session_state)
    if shared_sync_status is not None:
    ...
    if selected_menu == MENU_COMPARE:
        index_sync_result = IndexSyncResult(
